...
export default function ProductFamilyDetailPage() {
  ...
  const handleCreateProductSuccess = async (productId: string) => {
    ...
    try {
      // Asignar el producto recién creado a la familia
      const res = await fetch(`/api/product-families/${familyId}/products`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
      ...
      if (!res.ok) {
        showToast('error', data.error || 'Error al asignar el producto a la familia');
        return;
      }

      setShowCreateModal(false);
      fetchFamily();
      showToast('success', 'Producto creado y asignado a la familia correctamente');
    } catch (err) {
  ...
  return (
    <div>
      ...
      <CreateProductModal
        ...
        familyId={familyId}
        familyIsGeneral={family.isGeneral}
      />
    </div>
  );
}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="rounded-md bg-blue-50 border border-blue-200 p-3">
            <p className="text-sm text-blue-800">
              El producto se asignará automáticamente a esta familia al crearlo.
            </p>
          </div>
