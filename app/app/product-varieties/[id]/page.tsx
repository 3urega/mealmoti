...
export default function ProductVarietyDetailPage() {
  ...
  const handleCreateProductSuccess = async (productId: string) => {
    ...
    try {
      // Asignar el producto recién creado a la variedad
      const res = await fetch(`/api/products/${productId}/varieties`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (!res.ok) {
        showToast('error', data.error || 'Error al asignar el producto a la variedad');
        return;
      }
      ...
      fetchProducts();
      fetchVariety();
      showToast('success', 'Producto creado y asignado a la variedad correctamente');
    } catch (err) {
    ...
    <div>
      ...
      {variety && (
        <CreateProductModal
          ...
          varietyId={varietyId}
          familyIsGeneral={variety.subfamily.family.isGeneral}
        />
      )}
    </div>
  );
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="rounded-md bg-blue-50 border border-blue-200 p-3">
            <p className="text-sm text-blue-800">
              El producto se asignará automáticamente a esta variedad (y su subfamilia y familia) al crearlo.
            </p>
          </div>
