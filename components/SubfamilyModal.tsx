...
export default function SubfamilyModal({
  isOpen,
  onClose,
  familyId,
  subfamily,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = subfamily
        ? `/api/product-subfamilies/${subfamily.id}`
        : `/api/product-families/${familyId}/subfamilies`;
      const method = subfamily ? 'PUT' : 'POST';
      ...
      if (!res.ok) {
        ...
        setError(data.error || 'Error al guardar la subfamilia');
        setSaving(false);
        return;
      }
      ...
      <div className="w-full max-w-md rounded-lg bg-white p-6 shadow-xl">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-xl font-semibold text-gray-900">
            {subfamily ? 'Editar Subfamilia' : 'Nueva Subfamilia'}
          </h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex justify-end space-x-3">
            ...
            >
              {saving ? 'Guardando...' : subfamily ? 'Actualizar' : 'Crear'}
            </button>
          </div>
        </form>
      </div>
